import type { Metadata } from "next";
import Link from "next/link";
import { AdminPageHeader } from "@/components/admin/AdminPageHeader";
import { StatusBadge } from "@/components/admin/StatusBadge";
import { DeletePostForm } from "@/components/admin/DeletePostForm";
import { EmptyState } from "@/components/ui/EmptyState";
import { Pagination } from "@/components/ui/Pagination";
import { postDetailHref } from "@/components/content/PostCard";
import { getAllPosts } from "@/lib/services/posts";
import { getCategories } from "@/lib/services/categories";
import { getUserById } from "@/lib/services/users";
import { CONTENT_STATUSES, POST_TYPES, type ContentStatus, type PostType } from "@/lib/db/enums";
import { buildIdMap, formatDate, formatEnumLabel } from "@/lib/format";
import {
  inputClasses,
  buttonClasses,
  cn,
  tableWrapperClasses,
  tableClasses,
  tableHeadRowClasses,
  tableHeadCellClasses,
  tableRowClasses,
  tableCellClasses,
} from "@/lib/styles";

export const metadata: Metadata = {
  title: "Posts",
};

const PAGE_SIZE = 20;

function firstValue(value: string | string[] | undefined): string | undefined {
  return Array.isArray(value) ? value[0] : value;
}

export default async function AdminPostsPage({
  searchParams,
}: {
  searchParams: Promise<{ [key: string]: string | string[] | undefined }>;
}) {
  const params = await searchParams;
  const statusParam = firstValue(params.status);
  const status = CONTENT_STATUSES.includes(statusParam as ContentStatus)
    ? (statusParam as ContentStatus)
    : undefined;
  const typeParam = firstValue(params.type);
  const type = POST_TYPES.includes(typeParam as PostType) ? (typeParam as PostType) : undefined;
  const category = firstValue(params.category) || undefined;
  const search = firstValue(params.q) || undefined;
  const page = Math.max(1, Number(firstValue(params.page)) || 1);
  const deleted = params.deleted === "1";
  const deleteError = firstValue(params.deleteError);

  const [posts, categories] = await Promise.all([
    getAllPosts({ status, type, category, search, page, limit: PAGE_SIZE }),
    getCategories(),
  ]);

  const authorIds = [...new Set(posts.map((post) => String(post.author)))];
  const authors = await Promise.all(authorIds.map((id) => getUserById(id)));
  const authorNameById = new Map(
    authors.filter((author) => author).map((author) => [String(author!._id), author!.name])
  );
  const categoryNameById = buildIdMap(categories, "name");

  return (
    <div className="flex flex-col gap-6">
      <AdminPageHeader
        title="Posts"
        description="All posts regardless of publication status."
        action={
          <Link href="/admin/posts/new" className={buttonClasses("primary")}>
            New Post
          </Link>
        }
      />

      {deleted && (
        <p className="rounded-md border border-emerald-300 bg-emerald-50 px-4 py-3 text-sm text-emerald-700 dark:border-emerald-800 dark:bg-emerald-950 dark:text-emerald-300">
          Post deleted.
        </p>
      )}
      {deleteError && (
        <p className="rounded-md border border-red-300 bg-red-50 px-4 py-3 text-sm text-red-700 dark:border-red-800 dark:bg-red-950 dark:text-red-300">
          {deleteError}
        </p>
      )}

      <form
        action="/admin/posts"
        method="get"
        className="grid gap-3 rounded-lg border border-zinc-200 p-4 sm:grid-cols-5 dark:border-zinc-800"
      >
        <div className="sm:col-span-2">
          <label htmlFor="q" className="sr-only">
            Search by title
          </label>
          <input
            id="q"
            type="text"
            name="q"
            defaultValue={search}
            placeholder="Search by title"
            className={inputClasses}
          />
        </div>

        <div>
          <label htmlFor="status" className="sr-only">
            Status
          </label>
          <select id="status" name="status" defaultValue={status ?? ""} className={inputClasses}>
            <option value="">All statuses</option>
            {CONTENT_STATUSES.map((value) => (
              <option key={value} value={value}>
                {formatEnumLabel(value)}
              </option>
            ))}
          </select>
        </div>

        <div>
          <label htmlFor="type" className="sr-only">
            Type
          </label>
          <select id="type" name="type" defaultValue={type ?? ""} className={inputClasses}>
            <option value="">All types</option>
            {POST_TYPES.map((value) => (
              <option key={value} value={value}>
                {formatEnumLabel(value)}
              </option>
            ))}
          </select>
        </div>

        <div>
          <label htmlFor="category" className="sr-only">
            Category
          </label>
          <select id="category" name="category" defaultValue={category ?? ""} className={inputClasses}>
            <option value="">All categories</option>
            {categories.map((c) => (
              <option key={String(c._id)} value={String(c._id)}>
                {c.name}
              </option>
            ))}
          </select>
        </div>

        <button type="submit" className={cn(buttonClasses("secondary"), "sm:col-span-5 sm:w-fit")}>
          Apply filters
        </button>
      </form>

      {posts.length > 0 ? (
        <div className={tableWrapperClasses}>
          <table className={tableClasses}>
            <thead>
              <tr className={tableHeadRowClasses}>
                <th className={tableHeadCellClasses}>Title</th>
                <th className={tableHeadCellClasses}>Type</th>
                <th className={tableHeadCellClasses}>Category</th>
                <th className={tableHeadCellClasses}>Status</th>
                <th className={tableHeadCellClasses}>Author</th>
                <th className={tableHeadCellClasses}>Published</th>
                <th className={tableHeadCellClasses}>Updated</th>
                <th className={tableHeadCellClasses}>Actions</th>
              </tr>
            </thead>
            <tbody>
              {posts.map((post) => {
                const href = postDetailHref({ type: post.type, slug: post.slug });
                const canView = post.status === "PUBLISHED" && href;
                const id = String(post._id);

                return (
                  <tr key={id} className={tableRowClasses}>
                    <td className={cn(tableCellClasses, "max-w-xs truncate")}>{post.title}</td>
                    <td className={tableCellClasses}>{formatEnumLabel(post.type)}</td>
                    <td className={tableCellClasses}>
                      {post.category ? (categoryNameById.get(String(post.category)) ?? "—") : "—"}
                    </td>
                    <td className={tableCellClasses}>
                      <StatusBadge status={post.status} />
                    </td>
                    <td className={tableCellClasses}>{authorNameById.get(String(post.author)) ?? "—"}</td>
                    <td className={tableCellClasses}>{formatDate(post.publishedAt) ?? "—"}</td>
                    <td className={tableCellClasses}>{formatDate(post.updatedAt)}</td>
                    <td className={tableCellClasses}>
                      <div className="flex items-center gap-3">
                        <Link
                          href={`/admin/posts/${id}/edit`}
                          className="font-medium text-indigo-600 dark:text-indigo-400"
                        >
                          Edit
                        </Link>
                        {canView ? (
                          <Link
                            href={href}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="font-medium text-zinc-600 dark:text-zinc-400"
                          >
                            View
                          </Link>
                        ) : (
                          <Link
                            href={`/admin/posts/${id}/preview`}
                            className="font-medium text-zinc-600 dark:text-zinc-400"
                          >
                            Preview
                          </Link>
                        )}
                        <DeletePostForm id={id} title={post.title} />
                      </div>
                    </td>
                  </tr>
                );
              })}
            </tbody>
          </table>
        </div>
      ) : (
        <EmptyState
          title="No posts match these filters."
          description="Try clearing the filters, or create a new post."
        />
      )}

      <Pagination
        basePath="/admin/posts"
        currentPage={page}
        hasNextPage={posts.length === PAGE_SIZE}
        searchParams={{ status, type, category, q: search }}
      />
    </div>
  );
}
